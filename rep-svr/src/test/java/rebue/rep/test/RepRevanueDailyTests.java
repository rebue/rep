 ...
 */
@Slf4j
public class RepRevanueDailyTests {
    ...
    public void testCrud() throws IOException, ReflectiveOperationException {
        ...
        for (int i = 0; i < 20; i++) {
            mo = (RepRevanueDailyMo) RandomEx.randomPojo(RepRevanueDailyMo.class);
            mo.setId(null);
            log.info("添加报表-营收报表-日报的参数为：" + mo);
            final String addResult = OkhttpUtils.postByJsonParams(hostUrl + "/rep/revanue-daily", mo);
        ...
        final String listResult = OkhttpUtils.get(hostUrl + "/rep/revanue-daily?pageNum=1&pageSize=5");
        log.info("查询报表-营收报表-日报的返回值为：" + listResult);
        log.info("获取单个报表-营收报表-日报的参数为：" + mo.getId());
        final String getByIdResult = OkhttpUtils.get(hostUrl + "/rep/revanue-daily/get-by-id?id=" + mo.getId());
        log.info("获取单个报表-营收报表-日报的返回值为：" + getByIdResult);
        log.info("修改报表-营收报表-日报的参数为：" + mo);
        final String modifyResult = OkhttpUtils.putByJsonParams(hostUrl + "/rep/revanue-daily", mo);
        ...
        final String deleteResult = OkhttpUtils.delete(hostUrl + "/rep/revanue-daily?id=" + mo.getId());
